...
export function Scrollspy({ children, targetRef, onUpdate, className, offset = 0, smooth = true, dataAttribute = "scrollspy", history = true }) {
  ...
  useEffect(() => {
    // Query elements and store them in the ref
    if (selfRef.current) {
    ...
    // Check if there's a hash in the URL and scroll
    setTimeout(() => {
      scrollToHashSection();
      setTimeout(() => {
        handleScroll();
